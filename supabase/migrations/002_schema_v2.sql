-- ============================================================
-- LILAC APARTMENTS — Schema v2
-- Run in Supabase Dashboard → SQL Editor
-- ============================================================

-- 1. Add BHK type + private terrace to flats
...
-- 2. Residents (owner/tenant history with UPI IDs for auto-matching)
...
-- Seed current rates as of Mar-24 revision
...
-- 4. Corpus plans + per-flat installments
...
-- Seed Corpus 2025
WITH plan AS (
  INSERT INTO public.corpus_plans (name, description, total_target, pre_payments, planned_budget, status)
  VALUES (
    'Corpus 2025',
...
INSERT INTO public.corpus_plan_flats (plan_id, flat_id, target_amount, pre_payment, installment_1, installment_2, installment_3)
SELECT
  plan.id, f.id,
  CASE f.code
...
-- 5. App settings (drives dynamic fiscal year)
...
INSERT INTO public.app_settings (key, value) VALUES
  ...
  ('current_fiscal_month', 'May-26')
ON CONFLICT (key) DO NOTHING;

-- 6. Rewrite v_dues_tracker — dynamic FY from app_settings
CREATE OR REPLACE VIEW public.v_dues_tracker AS
WITH fy AS (
  SELECT value::integer AS fiscal_year FROM public.app_settings WHERE key = 'current_fiscal_year'
)
SELECT
  f.code              AS flat_code,
  f.block,
  ...
  COALESCE(SUM(t.amount) FILTER (
    WHERE t.fiscal_year = fy.fiscal_year AND t.cr_dr = 'CR'
...
GROUP BY f.code, f.block, f.flat_type, f.bhk_type, f.maintenance_amt, fy.fiscal_year
ORDER BY f.code;
...
CREATE POLICY "auth_read_residents"   ON public.residents FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth_write_residents"  ON public.residents FOR ALL    TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "auth_read_rate_hist"   ON public.maintenance_rate_history FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth_write_rate_hist"  ON public.maintenance_rate_history FOR ALL    TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "auth_all_corpus_plans"      ON public.corpus_plans      FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "auth_all_corpus_plan_flats" ON public.corpus_plan_flats FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "auth_all_app_settings"      ON public.app_settings      FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "auth_update_flats" ON public.flats FOR UPDATE TO authenticated USING (true);
